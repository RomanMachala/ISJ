...
def gen_quiz(qpool, *index, altcodes = ('A', 'B', 'C' ,'D' ,'E', 'F'), quiz = None):
    #Params: 
    # qpool     -list of questions and answers
    # index     -indexes (any quantity)
    # altcodes  -sequence, defaultly set to ABCDEF
    # quiz      -input quiz, defaultly set to None
    ...
    """
    if not quiz:
        quiz = []           #Creates new quiz (empty list) if not present
    
    for num in index:               #Iterates through indexes 
        try:
            question = qpool[num][0]            #Takes question from qpool
            answ = qpool[num][1]                #Takes answers from qpool fro better manipulation
        except IndexError as error:         
            print('Ignoring index ' + str(num) + ' - ' + str(error))   #If an exception is raised adn error is printed
        else:                   
            temp = []           #Temp list for better manipulation
            for zip_return in zip(altcodes, answ):       #Zip is a fnc that returns a tuple (combines first object to another first object from different lists), shorter list decides nu,ber of pairs returned                           
                zip_combined = zip_return[0] + ': ' + zip_return[1] #This is "object" in format: "Altcode: Answer"
                temp.append(zip_combined)                 #Append object from above in temp list
            final_object=(question, temp)           #Final object in format: "Question, [Altcode: Answer, ... ]"
            quiz.append(final_object)               #appends final object into quiz 
    return quiz     #Returns new quiz
